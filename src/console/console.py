...
def version(only_number: bool = False) -> None:
    ...
    if only_number:
        ...
        mzk.tsecho(f'MocaCommands ({core.VERSION})')
...
def stop(sleep: float = 0) -> None:
    ...
    mzk.sleep(sleep)
    pid = []
    for line in mzk.check_output('ps -ef | grep MocaCommands', shell=True).decode().splitlines():
...
def show_running_process() -> None:
    ...
    for line in mzk.check_output('ps -ef | grep MocaCommands', shell=True).decode().splitlines():
        items = line.split()
        if items[7].startswith('MocaCommands'):
...
@console.command('run-cmd')
def run_cmd(name: str, args: str = '') -> None:
    """Run registered commands."""
    cmd = core.commands.get(name, None)
    if cmd is None:
        mzk.tsecho('Unknown command.', fg=mzk.tcolors.RED)
    else:
        if "cmd" in cmd:
            mzk.call(f"{cmd['cmd']} {args}", shell=True)
        elif "cmd_path" in cmd:
            if cmd["cmd_path"].startswith("/"):
                mzk.call(f"{cmd['cmd_path']} {args}", shell=True)
            else:
                mzk.call(f"{core.COMMANDS_DIR.joinpath(cmd['cmd_path'])} {args}", shell=True)
        else:
            mzk.tsecho('Command format error.', fg=mzk.tcolors.RED)
